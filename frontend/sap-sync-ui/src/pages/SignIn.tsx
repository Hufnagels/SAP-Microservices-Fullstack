    ...
    <Box sx={{ display: 'flex', minHeight: '100vh' }}>
      ...
      <Box sx={{ display: { xs: 'none', sm: 'flex' }, flex: 1, position: 'relative', overflow: 'hidden', bgcolor: 'grey.900', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <Box sx={{ position: 'absolute', bottom: 48, left: 48, right: 48, color: 'white' }}>
          ...
          <Typography variant="body1" sx={{ opacity: 0.8 }}>{PAGE_SIGNIN_SUBTITLE}</Typography>
        </Box>
      </Box>
      ...
      <Box sx={{ width: { xs: '100%', sm: '420px' }, flexShrink: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', px: 5, py: 6, bgcolor: 'background.default', backgroundImage: 'linear-gradient(135deg, rgba(255,255,255,0.05) 25%, transparent 25%), linear-gradient(225deg, rgba(255,255,255,0.05) 25%, transparent 25%)' }}>
        <Box sx={{ width: '100%', maxWidth: 340 }}>
          ...
          <Box component="form" onSubmit={handleSubmit}>
            <TextField fullWidth label="Username" type="text" value={username} onChange={(e) => setUsername(e.target.value)} margin="normal" required />
              ...
              <OutlinedInput
                ...
                endAdornment={
                  <InputAdornment position="end">
                    <IconButton onClick={() => setShowPassword((s) => !s)} onMouseDown={(e) => e.preventDefault()} onMouseUp={(e) => e.preventDefault()} edge="end">
